...
担保物权所担保的债权的诉讼时效结束后，担保权人在诉讼时效结束后的二年内行使担保物权的，人民法院应当予以支持。
    ...
    （2）明确规定担保物权的存续期间是 主债权诉讼时效届满后 + 2年
...
>  主要是基于抵押人的求偿困境进行的修改
    ...
    （1）与担保法司法解释对所有的担保物权都作统一规定不同，
         ...
         （1.2）以占有、转移占有作为方法的：质权、留置权->
                220\237条
    （2）取消了 担保法司法解释第12条有关主债权诉讼时效届满后 + 2年->
...
> 一是从实用主义出发，事实上采取了抵押权消灭说，允许抵押人涂销登记；二是扩大了202条的适用范围，将其扩及以登记作为公示方法的权利质权。
      ...
      民法典草案在担保物权上与物权法一致，但在保证的规定上还有保证期间的规定->
...
> 涉及的是解释的问题
### (1.1)观点1：“英式浮动抵押”说/结晶说
    浮动抵押的最大特点在于其抵押权设立时，抵押财产尚不确定->
    并且在抵押财产固定（结晶）之前，不能对抗随后设立并登记的一般动产抵押权->
    当发生浮动抵押权和一般动产抵押权竞存的情形时->
    无论何者先登记，均应当认定一般动产抵押权优先于浮动抵押权。

### (1.2)观点2：“美式浮动抵押”
    浮动抵押的设立及其登记后的对抗效力，并不因抵押权设立及登记时抵押财产是否固定而异->
    对于浮动抵押登记时尚未固定的财产->
    如果嗣后有所增加，那么浮动抵押登记的效力也及于嗣后增加的财产之上。

### (1.3)本纪要规定：观点2
    (1)根据登记对抗的原理;
    (2)采取登记优先规则，有助于构建统一的动产抵押登记规则;
    (3)登记优先规则有助于提高浮动抵押制度的应用价值，推动浮动抵押制度的发展。
...
### （2.1）观点1：根据《担保法司法解释》第79条第1款的规定，不问设立先后、采何种公示方式，抵押权恒优先于质权
...
当事人未办理抵押物登记的，不得对抗第三人。当事人办理抵押物登记的，登记部门为抵押人所在地的公证部门。 
     ...
     根据《物权法》第 178 条规定的精神，担保法司法解释第 79 条第 1 款不再适用。
     cuz：《担保法司法解释》第79条第1款适用的前提已不复存在->
                 担保法司法解释是对担保法的解释->
                 担保法41、42、43室关于抵押登记的规定->
                 但是物权法对动产的抵押一概实行登记对抗->
                 未经登记，不得对抗善意第三人->
                 79的前提已经不复存在

### （2.2）观点2:应当参照适用《物权法》第199条的规定，根据是否完成公示以及公示先后情况来确定清偿顺序
> 本纪要采观点2       

### （2.3）主要规则
> depends 是否完成公示 + 公示的先后顺序
...
（三） 抵押权未登记的，按照债权比例清偿。
    ...
    （1）质权有效设立、抵押权也办理了登记的->
         此时应当参照适用《物权法》第199条的规定，根据公示先后来确定清偿顺序->
         质权设立在先的，质权人先受偿->
         抵押权登记在先的，抵押权人先受偿->
         抵押权和质权同一天设立的，视为顺序相同，按照债权比例清偿。
    （2）质权有效设立，抵押权未办理抵押登记的情况下->
         有效设立的质权优先于抵押权。
    （3）质权未有效设立，抵押权未办理抵押登记的->
         因此时抵押权已经有效设立，只是不登记没有对抗效力->
         抵押权为物权，质权未交付为债权->
         物权优先于债权->
         故抵押权优先受偿
...
（七） 法律、行政法规规定可以出质的其他财产权利。
    ...
    权利质权的范围限于：法律、行政法规允许的->
    抵押权、一般动产的范围是开放的，权利质权的范围是有范围的->
    新类型担保实际上不为法律行政法规认可，但为地方性的法规、规章认可、商事交易习惯认可->
    ie.新类型担保

## (1)含义
> 作为担保的标的物为新类型权利，如包括商铺租赁权、出租车经营权、排污权、信托收益权、以及高速公路收费权、景区收费权等各种收费权。
...
> 《行政许可法》9：依法取得的行政许可，除法律、法规规定依照法定条件和程序可以转让的外，不得转让。 
     ...
     （2.1）例外：涉及到特许经营权
             like:出租车经营权，需得到行政机关的许可->
                  合同两方之间存在的合同会涉及到违反行政许可法，因而无效的问题->
                  违法无效->
                  但符合一定条件下：有资格/经审批，合同有效

## (3)是否具有物权效力：看是否有法定的登记机构
> 涉及到如何理解 物权法定 的问题。
...
### （1）规则
    （1）原则上：有效
    （2）例外：若违反禁止流质、禁止流押条款部分无效->
                  like:约定一旦债务人到期不能履行债务，标的物由债权人所有
               部分无效不影响合同其他条款的效力->
               禁止归属性担保，并不禁止清算型的担保->
                   ie.届期不能履行债务，还是要进行清算

### （2）性质：是转让合同还是担保合同
      ie.是形式上让渡所有权，实质上达到担保的目的->
         是担保合同
...
### （2.1）观点 1：有效说
> 最高法麻锦亮认为采用观点 1
### （2.2）观点 2：无效说：虚伪意思表示说；违反物权法定说；流质条款说。
...
## （4）甲享有的是股权还在债权？
### （4.1）区分的意义
    （1）如果是股权，可以分红，可以参与经营管理；但股东也可能需要承担出逃出资责任，在公司破产是其权利要劣后于一般债权人。

### （4.2）区分标准
> 股权兼具人身权 & 财产权的属性。还涉及受让人是否是股东，是否应承担股东的权利和义务。
...
> 实践中，当事人还可能在前述基本交易模式基础上另行作出其他约定，只要不违反法律、行政法规的效力性强制 性规定，这些约定应当认定有效。
       ...
       (7)银行为保障自身利益，往往还会约定卖方要将货物交给由其指定的当事人监管 + 
...
> 在日常生活中，商业银行同其他金融行业是有合作的，来创造高额回报。银行资金经过证券公司、信托公司投入市场。这是活跃经营的一种表现。在我们具体的金融审判工作中，这是本会议纪要起草的主要背景，也是为什么会在本纪要中有这样的安排，也是与我们的工作有关。
    ...
    Q:房地产开发企业还不起钱，现在谁来起诉呢？商业银行自己起诉房地产开发企业。它能不能起诉房地产开发企业？
    Q:商业银行穿力那么多马甲向房地产企业发放贷款，它这样发放贷款与直接向房地产开发企业直接发放贷款的区别在哪里？
...
### （4） 场外配资合同被确认无效的法律后果
> 除依法取得融资融券资格的证券公司外，对其他个人或者单位与用资人的场外配资合同，无效。（but 违反账户实名制，有效。）违反特许经营许可的（主体不适格）【合同法司法解释10条】，合同无效。（放在整个法律体系下来解释）无效合同的利息赔偿，一定要提供资金配资的人没有任何好处。对法律规则的解释有一种叫历史解释，前面有，后面最新的意见磨掉了，表明现在不支持利息。现在配资无效的后果这里没有说，就是不给任何利息，是对此种扰乱金融秩序打击的惩罚。
